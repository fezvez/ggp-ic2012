...
<xsl:template name="main" match="/">  
  ...
  <style type="text/css" media="all"> 
    td.at {        width:  <xsl:value-of select="$width * 0.14"/>px; height: <xsl:value-of select="$height * 0.14"/>px;        border: 2px solid #000;        background-color: #CC88CC;        align: center;  valign: middle;    }      
    table.board {   background-color: #000000;  }      img.piece {        width:   <xsl:value-of select="$width * 0.9 * 0.14"/>px;        height:   <xsl:value-of select="$height * 0.9 * 0.1"/>px;              }   </style>        
  <!-- Draw Board -->    
  ...
  <xsl:choose>   
    <xsl:when test="//fact[relation='at' and   argument[2]=$row and argument[3]=$col and argument[1]='row_player']">  
      <img class="piece"   src="&ROOT;/resources/images/discs/yellow.png"/> 
    </xsl:when>  
    <xsl:when test="//fact[relation='at' and argument[2]=$row and   argument[3]=$col and argument[1]='col_player']"> 
