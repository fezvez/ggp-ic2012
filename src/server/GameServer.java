...
import server.threads.AbortRequestThread;
...
public final class GameServer extends Thread implements Subject
{
    ...
    public void run() {
        try {
            ...
            while (!stateMachine.isTerminal(currentState) && !aborting) {
     ...
     */
    private synchronized void sendAbortRequests() throws InterruptedException {
    	List<AbortRequestThread> threads =  new ArrayList<AbortRequestThread>(hosts.size());
    	
    	for (int i = 0; i < hosts.size(); i++) {
            threads.add(new AbortRequestThread(this, match, previousMoves, stateMachine.getRoles().get(i), hosts.get(i), ports.get(i), playerNames.get(i)));
        }
